...
module.exports =  (req) => {
  try {
    ...
    getIncorrectAtributtesBody(correctAtributtes,req,4,optionalAtributtes);
    const isNumber = /^\d+$/;
    ...
    if(content){
        ...
        if(!content.pdf?.content) throw new Error('Content is necessary,must be an url to pdf.')
        ...
        if(content.videos){
            ...
            content.videos.forEach(element => {
                ...
                if(element.content.length>20) throw Error(`${element.content} is a long content id.`);
            ...
            if(content.videos[totalvideos-1].order!=='Final_Video') throw new Error('Final_Video is neccesary')
        }
